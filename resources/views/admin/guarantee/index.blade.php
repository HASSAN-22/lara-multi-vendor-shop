                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <table class="table table-striped table-responsive-sm">
                            ...
                            <tbody>
                                @foreach($guarantees as $key=>$item)
                                    <tr>
                                        <th class="text-center">{{$key+1}}</th>
                                        <td class="text-center">{{$item->guarantee}}</td>
                                        <td class="flex align-center justify-center">
                                            <a href="{{route('admin.guarantee.edit',['guarantee'=>$item->id])}}" class="btn btn-info font-14">Edit</a>
                                            <form action="{{route('admin.guarantee.destroy',['guarantee'=>$item->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
